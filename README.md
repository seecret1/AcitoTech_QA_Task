...
<div class="bug-title">5. Сообщение «Вакансий пока нет» под фильтром «Команда» отображается, хотя результаты есть</div> <div class="priority priority-medium-label">Приоритет: MEDIUM</div> <div class="explanation"> <p><strong>Описание:</strong> Под фильтром «Команда» отображается серый блок «Вакансий пока нет». Однако на странице есть вакансии, и часть из них относится к командным ролям.</p> <p><strong>Причина medium приоритета:</strong> Вызывает путаницу и может восприниматься как системная ошибка, заставляя пользователя подумать, что фильтр не сработал или вакансий действительно нет.</p> </div>
<div class="bug-title">6. Геофильтрация не работает</div> <div class="priority priority-high-label">Приоритет: HIGH</div> <p><strong>Описание:</strong> Пользователь выбирает «Санкт-Петербург» в фильтре, но система показывает вакансии из Москвы, Самары и других городов.</p> <p><strong>Причина high приоритета:</strong> Геофильтр является одним из ключевых для соискателей. Ошибка полностью нарушает корректность поиска.</p>
<div class="bug-title">7. Невозможная комбинация фильтров: «Стажировка» и «Руководящая роль» включены одновременно</div> <div class="priority priority-low-label">Приоритет: LOW</div> <div class="explanation"> <p><strong>Описание:</strong> Эти параметры взаимоисключающие. Интерфейс не уведомляет пользователя и не блокирует такую комбинацию.</p> <p><strong>Причина low приоритета:</strong> Логическая ошибка в UX, не влияющая напрямую на поиск, но создающая неправдоподобный сценарий.</p> </div>
<div class="bug-title">8. Несогласованность в написании условий работы</div> <div class="priority priority-low-label">Приоритет: LOW</div> <div class="explanation"> <p><strong>Описание:</strong> В карточках вакансий используются разные формулировки удалённости:<br> – «можно удалённо»<br> – «удалённо»<br> – «офис и удалённо»<br> – отсутствие любого указания<br> </p> <p><strong>Причина low приоритета:</strong> Не критично для работы поиска, но снижает целостность интерфейса и усложняет визуальное сканирование списка.</p> </div>
